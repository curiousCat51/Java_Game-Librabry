class Player extends Creature{
  ...
  void move(){
    if(EventListener.getKeyUP()){
      move(CreatureDirections.HOCH, image_left, PLAYER_SPEED);
    }
    
    if(EventListener.getKeyDOWN()){
      move(CreatureDirections.RUNTER, image_right, PLAYER_SPEED);
    }
    
    if(EventListener.getKeyLEFT()){
      move(CreatureDirections.LINKS, image_left, PLAYER_SPEED);
    }
    
    if(EventListener.getKeyRIGHT()){
      move(CreatureDirections.RECHTS, image_right, PLAYER_SPEED);
    }
